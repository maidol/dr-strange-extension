...
以 `deps@1.0.0` 列入。其中两个字段是发布工作流不会代做的判断，值得写明：

| 字段 | 取值 | 缘由 |
|---|---|---|
| `claims` | `build manifests` | 此处该列是散文，与 `git` 相同，因为这两个插件都不经由扩展名抵达 |
| `min_drsg` | `2.7.0` | 把清单文件名路由到本插件的那个版本。更旧的宿主永远不会把任何东西交给它，那样这条目就成了一个无法兑现的承诺 |

更旧的宿主不会隐藏该条目——它会照常列出，并说明 `needs drsg >= 2.7.0, this is
<你的版本>`：一份悄悄略去某个插件的目录，只会让你去排查 `drsg plugin install`
为何从不提供它。只要宿主讲的是 `1.0.0` 契约，该构件都可按 URL 安装；只是在宿主
学会把清单交给它之前，它无从读起。
