...
- **方法调用**（`.read()`）不写路径；它能解析到什么程度，取决于接收者的
  类型能否**从函数体里读出来**——参数或 `let` 的类型标注、字段的声明类型、
  构造路径（`Vec::new()`）、声明的返回类型，以及它们的链式组合
  （`self.items.iter().map(…)`），`?`/`.unwrap()` 剥开声明的
  `Result`/`Option`。本树声明的类型落到它自己的方法上；本树未声明的类型
  ——std 的、依赖的——落到键为 `Type::method` 的外部替身上
  （`Vec::push`、`str::trim`、`Iterator::map`），这就是关于它所知的全部。
  没有任何东西陈述其类型的接收者，以及泛型参数，**计数，绝不猜测**。
