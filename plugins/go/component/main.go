...
func (goPlugin) Describe() ext.Manifest {
	return ext.Manifest{
		...
		// type as a `USES_TYPE` edge rather than leaving it in a prop, and
		// records where each declaration ends. A plane holding `go@1` facts
		// is still readable; it simply has none of these, and
		// `_generated_by` says which version wrote what.
		Version:    "2",
